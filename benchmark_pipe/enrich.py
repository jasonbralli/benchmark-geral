...
def _load_aa_metrics_map() -> dict[str, dict[str, float | None]] | None:
    """Carrega mapa rico slug -> {aa_index, speed_tps, ttft_s, e2e_s, coding, agentic}."""
    try:
        from .aa_api import CACHE as AA_CACHE, build_aa_metrics_map
        if AA_CACHE.exists():
            raw = json.loads(AA_CACHE.read_text(encoding="utf-8"))
            m = build_aa_metrics_map(raw)
            if m:
                return m
    except Exception as e:  # noqa: BLE001
        logger.debug("AA metrics map nao carregado: %s", e)
    return None
...
def enrich_unified(models: list[UnifiedModel], aa_api_map: dict[str, float] | None = None) -> list[UnifiedModel]:
    ...
    metrics_map = _load_aa_metrics_map()
    ...
    lookup_metrics_fn = None
    if metrics_map:
        try:
            from .aa_api import lookup_aa_metrics as _lm
            lookup_metrics_fn = _lm
        except Exception:
            lookup_metrics_fn = None
    ...
    for m in models:
        if lookup_fn is not None and aa_api_map is not None:
            ...
            if idx is not None:
                ...
                # Propaga métricas ricas (speed/ttft/e2e/coding/agentic)
                if lookup_metrics_fn is not None and metrics_map:
                    rich = lookup_metrics_fn(m.model_id, m.canonical_id, metrics_map)
                    if rich:
                        if m.speed_tps is None:
                            m.speed_tps = rich.get("speed_tps")
                        if m.ttft_s is None:
                            m.ttft_s = rich.get("ttft_s")
                        if m.e2e_s is None:
                            m.e2e_s = rich.get("e2e_s")
                        if m.coding_index is None:
                            m.coding_index = rich.get("coding_index")
                        if m.agentic_index is None:
                            m.agentic_index = rich.get("agentic_index")
                continue
